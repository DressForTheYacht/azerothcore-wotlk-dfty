...
void LearnSpells::MaxAllSkills(Player* player)
{
    ...
    for (uint32 skillId = 0; skillId < sSkillLineStore.GetNumRows(); ++skillId)
    {
        ...
        // Only try to set skills the player already knows
        if (!player->HasSkill(skillLine->id))
            continue;

        uint16 max = player->GetMaxSkillValue(skillLine->id);
        player->SetSkill(skillLine->id, 0, max, max);
    }

    // Give DeathKnight money
    if (player->getClass() == 6)
    {
        player->ModifyMoney(900000);
    }
}
